...
(async function () {
  try {
    ...
    /* 33 這一次:`263eff4`(AI 按鈕,把一句話或一張照片變成願望／行程／座位)
       帶進了 `已加進許願:`、`已加到 Day `、`已更新 ` 三句。
       它們是**寫入成功的回饋**,而這一套本來就有 —— `已連上 Notion`、`已清除`、
       `已匯入 ` 都在頁尾用同一條路報同一件事。**同類,不是第四套。**
    ...
    /* ---- 這個對話框只有「取消」關得掉 ----
       ...
       **問的是第一張卡(還沒送出去那張)**:唯讀的人送出之後就直接進許願框了,
       那張要選類型的卡他看不到 —— 但「只有取消關得掉」是整個對話框的規則,
       在哪一張卡上都該成立。
